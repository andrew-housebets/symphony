defmodule SymphonyElixir.CoreTest do
  ...
  test "config defaults and validation checks" do
    ...
    write_workflow_file!(Workflow.workflow_file_path(), tracker_paused_states: "Human Review, Waiting")
    assert Config.linear_paused_states() == ["Human Review", "Waiting"]
  ...
  test "current WORKFLOW.md file is valid and complete" do
    ...
    assert is_map(hooks)
    assert Map.get(hooks, "after_create") =~ "go mod download"
    assert Map.get(hooks, "after_create") =~ "npm --prefix frontend ci --legacy-peer-deps"
    assert Map.get(hooks, "after_create") =~ "npm --prefix backoffice ci"
  ...
  test "reconcile stops running issue when it moves into paused review state" do
    write_workflow_file!(Workflow.workflow_file_path(),
    ...
    issue = %Issue{
      ...
      description: "Should not execute Codex turns",
      labels: []
    }
  ...
  test "normal worker exit schedules active-state continuation retry" do
    ...
    continuation_retry_ms = Config.continuation_retry_ms()
    assert_due_in_range(due_at_ms, max(continuation_retry_ms - 1_000, 0), continuation_retry_ms + 1_500)
  end
  ...
  test "in-repo WORKFLOW.md renders correctly" do
    ...
    assert prompt =~ "start every ticket pickup with red-green-refactor discipline"
  ...
  test "agent runner stops codex session when update recipient exits" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "symphony-elixir-agent-runner-recipient-down-#{System.unique_integer([:positive])}"
      )
    ...
    try do
        ...
        while IFS= read -r line; do
          ...
          case "$count" in
            1)
              ...
              ;;
            2)
              printf '%s\\n' '{\"id\":2,\"result\":{\"thread\":{\"id\":\"thread-recipient\"}}}'
              ;;
            3)
              printf '%s\\n' '{\"id\":3,\"result\":{\"turn\":{\"id\":\"turn-recipient\"}}}'
              ;;
            4)
              printf '%s\\n' '{\"method\":\"turn/updated\"}'
              sleep 30
      ...
      issue = %Issue{
        id: "issue-recipient-down",
        identifier: "MT-199",
        title: "Stop when recipient exits",
        description: "Ensure session exits when orchestrator dies",
        state: "In Progress",
      ...
      assert_raise RuntimeError, ~r/Agent run failed/, fn ->
        AgentRunner.run(issue, recipient, issue_state_fetcher: fn [_issue_id] -> {:ok, [%{issue | state: "Done"}]} end)
      end
    after
      File.rm_rf(test_root)
    end
